<template>
  <div v-if="currentCustomer">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="last_NAME" required name="last_NAME" v-model="currentCustomer.last_NAME">
    </div>
    <div class="mb-3">
...
export default {
  ...
  methods: {
    ...
    updateCustomer() {
      // axios 공통함수 호출
      CustomerService.update(this.currentCustomer.cid, this.currentCustomer)
          // 성공하면 then() 결과가 전송됨
          .then(response => {
            console.log(response.data);
    ...
    deleteCustomer() {
      // axios 공통함수 호출
      CustomerService.delete(this.currentCustomer.cid)
          // 성공하면 then() 결과가 전송됨
          .then(response => {
            console.log(response.data);
